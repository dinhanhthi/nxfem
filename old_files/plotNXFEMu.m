function plotNXFEMu(nS,eS,mshs,lsf,msh,iPs)
...
if lsf{2} % user wanna plot level set function
    ...
    elseif lsf{3}==3 % 3D
        ...
        pdeplot(points,edges,triangles,'XYData',lsf{1},'Title','uh');
        plotMesh(lsf{4},msh); % plot with mesh
        plotGamh(iPs); % plot gam_h
...
if eS{2} % user wanna plot exact solution
    figure(nfg); nfg = nfg+1;
    if eS{3}==2 % 2D
        pdeplot(points,edges,triangles,'XYData',eS{1},'Title','uex');
...
if nS{2} % user wanna plot numerical solution
    figure(nfg);
    if nS{3}==2 % 2D
        pdeplot(points,edges,triangles,'XYData',nS{1},'Title','uh');
