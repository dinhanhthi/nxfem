...
savePlot = 1; % wanna save plot or not?
    pathOption = '_CR';
    testCase = 'Test the CR to insert results to thesis (u depends on t)'; % note for info_and_errors.txt
withMesh = false;
...
phi0 = phi;
if ~isempty(CTs)
    normPhih0oGh0 = getNormL2oGstd(msh,pa,CTs,CT,phi0); % initial phi
...
%     delOld = ones(1,size(msh.t,2))*hTmax/2; % just for testing
%     delNew = delOld;
...
normPhih0PhionOmg = getNormL2fhf(msh,pa,phi0,model.defPhi); % ||phi_h^0 - phi^0||_L2(Omg)
...
%% display results
fprintf('hTmax: %0.7f,\n',msh.hTmax);
fprintf('||phi_h^0||_{L2(Gam_h^0)}: %0.7f,\n',normPhih0oGh0);
fprintf('||phi_h^N||_{L2(Gam_h^N)}: %0.7f,\n',normPhihNoGhN);
fprintf('||phi_h^N-phi_h^0||_{L2(Omg)}: %0.7f\n',normErrPhihN);
fprintf('||phi^0||_L2(Gam_h^N): %0.7f\n',normPhionGhN);
fprintf('||phi^0||_L2(Gam_h^0): %0.7f\n',normPhionGh0);
fprintf('||phi_h^0-phi^0||_{L2(Omg)}: %0.7f\n',normPhih0PhionOmg);
fprintf('||phi_h^N-phi^0||_{L2(Omg)}: %0.7f\n',normPhihNPhionOmg);



%% save error result to file
if savePlot
    fileName = strcat(path_test_result,'/info_and_errors.txt');
    fileID = fopen(fileName,'w');
        ...
        fprintf(fileID,'||phi^0||_L2(Gam_h^N): %0.7f\n',normPhionGhN);
        fprintf(fileID,'||phi^0||_L2(Gam_h^0): %0.7f\n',normPhionGh0);
