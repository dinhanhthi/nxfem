function nf = plotNXFEM(msh,iPs,cnf,varargin)
...
%   - `withGamh`: plot Gam_h or not? (interface): false (default) or true
...
addRequired(p,'cnf'); % number of plot
...
addParameter(p,'export',0,@islogical); 
    % export to a .jpg file with the name the same with title
parse(p,msh,iPs,cnf,varargin{:});
