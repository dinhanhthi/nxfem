function [ii,jj,vv1,vv2,vv3,vv4] = getTripleGnGnoG(CTs,iPs,msh,L)
% Find triplet for + int_Gam L*{grad_n}{grad_n}
% First used to find norm ||{gran_n}||_{-1/2}
...
for t=1:nCTs
    ...
    for i=1:3
        for j=1:3
            ...
            % A_{ij}=a(phi_j,phi_i)
            % (grad_ni * grad_nj)
            gNgN = getGradnPhi(j,mt,pointA,pointB,msh)*...
                            getGradnPhi(i,mt,pointA,pointB,msh);
            vv1(idx) = L(1,t)*gNgN; % A_ij
            vv2(idx) = L(2,t)*gNgN; %A_k(i)k(j)
            vv3(idx) = L(3,t)*gNgN; % A_k(i)j
            vv4(idx) = L(4,t)*gNgN; % A_ik(j)
            idx = idx+1;
        end % end for j
    end % end for i
end % end for t
