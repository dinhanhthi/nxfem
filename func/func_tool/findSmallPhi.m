...
%         - cut triangles transform from CTs to NCTs (t2Omg1, t2Omg2): indices in OLG CTs
...
for t=1:nCTs % consider all cut triangles
    ...
    for i=1:3 % consider all 3 vertices of each triangle
        if typeCTs(t)==2 % 2 nodes in Omg1, 1 node in Omg2
            ...
            normPhiOmg2 = getTriplePPPart(msh,pa,triangle,i,i,iP1,iP2,rV,K);
            normWhole = getTriplePPWhole(msh,pa,triangle,i,i,K); % ||.||^2
            normPhiOmg1 = normWhole - normPhiOmg2; % ||.||^2
        else % typeCT = 0 or 4
            rV = points(:,nodeCTs.eachOmg1(1,t)); % the only vertex in Omg1
            normPhiOmg1 = getTriplePPPart(msh,pa,triangle,i,i,iP1,iP2,rV,K);
            normWhole = getTriplePPWhole(msh,pa,triangle,i,i,K);
...
tri2del = [t2Omg1,t2Omg2]; % triangles to be deleted
end