function interPoints = getiPs(cutTriangles,phi,msh,pa)
...
for i=1:nCT
    ...
    if negative(phiTri(1),phiTri(2),tol) % edge 1
        cP(i) = cP(i)+1;
        temp(:,cP(i),i) = getiPsonE(pt(:,1),pt(:,2),phiTri(1),phiTri(2));
    end
    if negative(phiTri(2),phiTri(3),tol)||equal0(phiTri(2),phiTri(3),tol) % edge 2
        cP(i) = cP(i)+1;
        temp(:,cP(i),i) = getiPsonE(pt(:,2),pt(:,3),phiTri(2),phiTri(3));
    end
    if negative(phiTri(3),phiTri(1),tol)||equal0(phiTri(3),phiTri(1),tol) % edge 3
        cP(i) = cP(i)+1;
        temp(:,cP(i),i) = getiPsonE(pt(:,3),pt(:,1),phiTri(3),phiTri(1));
    end
end
