...
savePlot = 1; % wanna save plot or not?
    testCase = '5'; % count the test and used to name the folder
    pathOption = '_findGood';
    moreInfo = 'TesT 5: find best for chopp06 ([R]restart). Like test 4 but decrease bcu3.'; % write inside file txt
...
useFFmesh = 1; % use mesh generated by freefem++?
    ...
    nSeg = 35;  % mesh settings (only for useFFmesh=0)
...
%         clear CTs NCTs NCTs2 nodeCTs areaChildCTs iPs; % just in case (bad practice)
    ...
    if ~isempty(DNodes)
        uold(DNodes,1) = pa.bcu3; % dynamic Dirichlet bc
    else
