...
savePlot = 1; % wanna save plot or not?
    testCase = '17c'; % count the test and used to name the folder
    pathOption = 'findGood';
    moreInfo = 'TesT 17c: find best for chopp06 ([R]restart). After changing to FreeNodes.'; % write inside file txt
...
pa.useGP = 1; % wanna use ghost penalty term?
    pa.gam1 = 1e-3; % parameter for 1st term
    pa.gam2 = 1e-3 ; % parameter for 2nd term
...
cpV.lamH = 1e9; % penalty coefficient for v (potential)
...
if savePlot
   % Save parameters' info to file
    fileName = strcat(path_test_result,'/parameters_mesh',testCase,'.txt');
