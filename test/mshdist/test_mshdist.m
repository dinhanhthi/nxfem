...
reguMesh = 0; % use regular mesh or not?
useFMM = 1;
useSUPG = 1;
...
phiWrong = model.phiWrong(x,y,pa); % 1 x number of points (row array)
phiRight = model.phiRight(x,y,pa); % 1 x number of points (row array)
% phi(abs(phi)<pa.tol)=0; % find phi which are very small (~0) and set to 0



%% triangles info
tris = getTriangles(phiWrong,msh,pa); % tris has 3 factors (structure var)
CTs=tris.CTs; NCTs1=tris.NCTs1; NCTs2=tris.NCTs2;
msh.nStd = size(points,2); % number of standard nodes
CT = getInfoCTs(CTs,phiWrong,msh,pa); % CT has many factors (structure var)
...
if showPlot
    ...
    nf = plotNXFEM(msh,iPs,nf,phiWrong,'withMesh',true,'title',titlePlot1,'dim',3,'export',false); % phi
end
...
if useFMM
    ...
    mshdist_w_sol(msh,phiWrong,path_phi,'phi_test'); % export to .sol
    system(call_mshdist); % run 'mshdist file/to/phi' (redistancing)
    phiWrong = mshdist_r_sol(phiWrong,path_phi,'phi_test'); % update phi
    
    %% plot phiFixed
    if showPlot
        titlePlot3 = 'PhiWrong (fixed)';
        nf = plotNXFEM(msh,iPs,nf,phiWrong,'withMesh',true,'title',titlePlot3,'dim',3,'export',false); % phi
    end
%     
%     nNormGP = nNormGP+1;
%     norm_gradphi(nNormGP) = getNormL2Gstd(msh,phi);
...
%% triangles info
tris = getTriangles(phiRight,msh,pa); % tris has 3 factors (structure var)
CTs=tris.CTs; NCTs1=tris.NCTs1; NCTs2=tris.NCTs2;
msh.nStd = size(points,2); % number of standard nodes
CT = getInfoCTs(CTs,phiRight,msh,pa); % CT has many factors (structure var)
nodeCTs=CT.nodes; areaChildCTs=CT.areaChild;iPs=CT.iPs;

%% plot phiRight
if showPlot
    titlePlot3 = 'phiRight';
    nf = plotNXFEM(msh,iPs,nf,phiRight,'withMesh',true,'title',titlePlot3,'dim',3,'export',false); % phi
end
